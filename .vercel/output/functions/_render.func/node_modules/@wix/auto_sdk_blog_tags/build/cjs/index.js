...
async function typedQueryTags(query, options) {
  const { httpClient, sideEffects } = arguments[2];
  const payload = (0, import_rename_all_nested_keys.renameKeysFromSDKRequestToRESTRequest)({
    query,
    ...options
  });
  const reqOpts = queryTags(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
...
function typedQueryTags2(httpClient) {
  return (query, options) => typedQueryTags(
    query,
    options,
    // @ts-ignore
    { httpClient }
...
function customQueryTags(httpClient) {
  const router = (0, import_query_method_router.createQueryOverloadRouter)({
    builderQueryFunction: (options) => queryTags3(httpClient)(options),
    typedQueryFunction: (query, options) => typedQueryTags2(httpClient)(query, options),
...
var queryTags4 = /* @__PURE__ */ (0, import_rest_modules3.createRESTModule)(customQueryTags);
