...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": "Blog", "description": "Semua artikel, tutorial, dan tulisan Tim Seputar Redaksi seputar pertanian (Agriculture) dan IoT." }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<section class="py-16 sm:py-20"> <div class="max-w-6xl mx-auto px-6"> <div class="mb-10"> <h1 class="text-3xl sm:text-4xl font-bold text-gray-900 mb-3">Blog</h1> <p class="text-gray-500">Artikel, tutorial, dan pemikiran seputar pertanian (Agriculture) dan IoT.</p> </div> ${renderComponent($$result2, "CategoryFilter", $$CategoryFilter, { "categories": categories })} ${posts.length > 0 ? renderTemplate`<div class="grid gap-10 sm:grid-cols-2"> ${posts.map((post) => renderTemplate`${renderComponent($$result2, "ArticleCard", $$ArticleCard, { ...post })}`)} </div>` : renderTemplate`<div class="text-center py-20"> <p class="text-gray-500 text-lg">Belum ada artikel yang dipublikasikan.</p> </div>`} </div> </section> ` })}`;
